...
if __name__ == "__main__":
    ...
    p.resetBasePositionAndOrientation(robotId, [0,0,0], p.getQuaternionFromEuler((np.pi/180)*np.array([0,0,0]))) # Staring position of robot
    ...
    for i in range (simSteps): #Time to run simulation
        ...
        if step in range(int(0/timeStep), int(20/timeStep)):
            ...
            if step > (ctrlUpdateStep + ctrlStep):
                ...
                # if step in range(int(5/timeStep), int(15/timeStep)):
                #     w = [w[0], w[0],w[0], w[0] ]
