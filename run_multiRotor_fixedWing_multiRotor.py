...
physicsClient = p.connect(p.GUI, options="--mp4=\"MultiRotorFixedWingMultiRotor.mp4\" --mp4fps==30")#or p.DIRECT for non-graphical version
...
if __name__ == "__main__":
    ...
    for i in range (simSteps): #Time to run simulation
        ...
        if step in range(int(0/timeStep), int(5/timeStep)):
            ...
            des_yawW = 0
            des_orientationW = p.getQuaternionFromEuler([0,-1.3,3.14]) # [roll, pitch, yaw]
            des_positionW = [-40,0,20] 
            des_velocityW = [0,0,0]
        ...
        if step in range(int(5/timeStep), int(10/timeStep)):
            ...
            des_yawW = 0
            des_orientationW = p.getQuaternionFromEuler([0,-1.3,3.1415]) # [roll, pitch, yaw]
        ...
        if step in range(int(10/timeStep), int(20/timeStep)):
            hingeAngle = 1.57
            frameState = "quadrotor"
