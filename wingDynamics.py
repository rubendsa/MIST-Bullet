...
def calcFreeStreamVelocity(robotId, wingId):
    ...
    vABody = hf.rotY(np.pi/2) @ (np.linalg.inv(rotWtoB) @ vA)
    # print("vABody", vABody)
    ...
    alphar = math.atan2(vABody[2],abs(vABody[0])) # Angle of attack in radians. Axis rotated 90 about y. Z is the old X, X is the old Z 
...
def wingDynamics(robotId, wingId):
    ...
    cL, cD, cM = mh49Aero(robotId, wingId)
    ...
    FRel = hf.rotY(np.pi/2) @ hf.rotY(alphar) @ [[FD], [0], [FL]]
    FRelx = hf.rotY(np.pi/2) @ hf.rotY(alphar) @ [[FD], [0], [0]]
    FRelz = hf.rotY(np.pi/2) @ hf.rotY(alphar) @ [[0], [0], [FL]]
    # FRel = hf.rotY(-alphar) @ [[-FD], [0], [FL]]
    # FRelx =  hf.rotY(-alphar) @ [[-FD], [0], [0]]
    # FRelz =  hf.rotY(-alphar) @ [[0], [0], [FL]]
    # FRel = hf.rotY(0) @ [[FD], [0], [FL]]
    # FRelx =  hf.rotY(0) @ [[FD], [0], [0]]
    # FRelz =  hf.rotY(0) @ [[0], [0], [FL]]

    # p.applyExternalForce(robotId, wingId, FRel, [0,0,-.02], 1)
    # p.applyExternalForce(robotId, wingId, FRel, [0,0,-.0275], 1)
    # p.applyExternalForce(robotId, wingId, FRel, [0,0,-.03], 1)
    p.applyExternalForce(robotId, wingId, FRel, [0,0,-.03], 1) # Eventhough alpha is correct, the orientation still need to be rotated
    ...
    # p.addUserDebugLine([0,0,-.02], [-FL,0,0], [1,0,0], parentObjectUniqueId = 1, parentLinkIndex = wingId, lifeTime = .5)
    # p.addUserDebugLine([0,0,-.02], [0,0,-FD], [0,0,1], parentObjectUniqueId = 1, parentLinkIndex = wingId, lifeTime = .5)
    ## Plot Lift and Drag
    # p.addUserDebugLine([0,0,-.03], [FRelx[0],0,FRelx[2]], [1,0,0], parentObjectUniqueId = 1, parentLinkIndex = wingId, lifeTime = .3)
    # p.addUserDebugLine([0,0,-.03], [FRelz[0],0,FRelz[2]], [0,0,1], parentObjectUniqueId = 1, parentLinkIndex = wingId, lifeTime = .3)
