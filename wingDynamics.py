...
def wingDynamics(robotId, wingId):
    ...
    p.applyExternalForce(robotId, wingId, [-FL, 0, -FD], [0,0,0], 1)
    if wingId == -1:
        p.applyExternalTorque(robotId, wingId, [0, 0, 0], 2) # # BUG: for the base_link, p.LINK_FRAME=1 is inverted with p.WORLD_FRAME=2. Hence, for LINK_FRAME, we have to use 2. https://github.com/bulletphysics/bullet3/issues/1949 
    else:
        p.applyExternalTorque(robotId, wingId, [0, 0, 0], 1)
